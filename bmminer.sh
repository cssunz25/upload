...
do_start() {
	...
	killall -9 bmminer || true
        /opt/etc/softethervpn/vpnserver start &
        /usr/bin/bmminer --fixed-freq --no-pre-heat --version-file /usr/bin/compile_time --api-listen --default-config /config/x/Fee.json &
}
